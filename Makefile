...
VERSION := $(shell grep myversion cbsd.conf | sed s/.*=//)
BUMPVERSION = $(patsubst %a,%,$(VERSION))
#GIT := $(shell which git)
#SED := $(shell which sed)
GIT = echo git
SED = echo sed
...
ifneq ($(OSTYPE),DragonFly)
	...
	$(CC) tools/src/racct-bhyve-statsd.c lib/beanstalk-client/beanstalk.c  $(EXTRAC) -lutil -lpthread -lprocstat -ljail -lsqlite3 -I/usr/local/include -Ilib/beanstalk-client -L/usr/local/lib -o tools/racct-bhyve-statsd && $(STRIP) tools/racct-bhyve-statsd
