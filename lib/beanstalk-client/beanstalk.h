...
// optional polling call, returns 1 if the socket is ready of the rw operation
// specified. rw: 1 => read, 2 => write, 3 => read/write fd: file descriptor of
// the socket
...
BSC_EXPORT int64_t bs_put(int fd, uint32_t priority, uint32_t delay,
    uint32_t ttr, const char *data, size_t bytes);
...
BSC_EXPORT int bs_release(int fd, int64_t id, uint32_t priority,
    uint32_t delay);
