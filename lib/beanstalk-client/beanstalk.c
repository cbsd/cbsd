...
const char *bs_status_verbose[] = { "Success", "Operation failed",
	"Expected CRLF", "Job too big", "Queue draining", "Timed out",
	"Not found", "Deadline soon", "Buried", "Not ignored" };
...
const char *
bs_status_text(int code)
{
	unsigned int cindex = (unsigned int)abs(code);
	return (cindex > sizeof(bs_status_verbose) / sizeof(char *)) ?
	    0 :
		  bs_status_verbose[cindex];
}
...
void
bs_start_polling(bs_poll_function f)
{
	bs_poll = f;
}
...
BSMP *
bs_message_packet_new(size_t bytes)
{
...
void
bs_message_packet_append(BSMP *packet, const char *data, size_t bytes)
{
	if (packet->offset + bytes > packet->size) {
		packet->data = (char *)realloc(packet->data,
		    packet->size + bytes);
		assert(packet->data);
		packet->size += bytes;
	}
...
void
bs_message_packet_free(BSMP *packet)
{
	free(packet->data);
	free(packet);
}
...
int
bs_watch(int fd, const char *tube)
{
	BSM *message;
	char command[1024] = { 0 };
...
int
bs_ignore(int fd, const char *tube)
{
	BSM *message;
	char command[1024] = { 0 };
...
int64_t
bs_put(int fd, uint32_t priority, uint32_t delay, uint32_t ttr,
    const char *data, size_t bytes)
{
	...
	if (BS_STATUS_IS(message->status, bs_resp_inserted)) {
		id = strtoll(message->status + strlen(bs_resp_inserted) + 1,
		    NULL, 10);
		bs_free_message(message);
		return id;
	}
	...
	if (BS_STATUS_IS(message->status, bs_resp_buried)) {
		id = strtoll(message->status + strlen(bs_resp_buried) + 1, NULL,
		    10);
		bs_free_message(message);
		return id;
	}
	...
	BS_RETURN_FAIL_WHEN(message, bs_resp_job_too_big,
	    BS_STATUS_JOB_TOO_BIG);
	BS_RETURN_FAIL_WHEN(message, bs_resp_draining, BS_STATUS_DRAINING);
	BS_RETURN_INVALID(message);
}

int
bs_delete(int fd, int64_t job)
{
	BSM *message;
	char command[512] = { 0 };
...
int
bs_reserve_job(int fd, char *command, BSJ **result)
{
	BSJ *job;
	BSM *message;
	...
	if (BS_STATUS_IS(message->status, bs_resp_reserved)) {
		*result = job = (BSJ *)malloc(sizeof(BSJ));
		...
		sscanf(message->status + strlen(bs_resp_reserved) + 1,
		    "%" PRId64 " %lu", &job->id, &job->size);
...
int
bs_reserve_with_timeout(int fd, uint32_t ttl, BSJ **result)
{
	char command[512] = { 0 };
...
int
bs_release(int fd, int64_t id, uint32_t priority, uint32_t delay)
{
	BSM *message;
	char command[512] = { 0 };

	snprintf(command, 512,
	    "release %" PRId64 " %" PRIu32 " %" PRIu32 "\r\n", id, priority,
	    delay);
	BS_SEND(fd, command, strlen(command));
...
int
bs_bury(int fd, int64_t id, uint32_t priority)
{
	BSM *message;
	char command[512] = { 0 };
...
int
bs_peek_job(int fd, char *command, BSJ **result)
{
	BSJ *job;
	BSM *message;
	...
	if (BS_STATUS_IS(message->status, bs_resp_found)) {
		*result = job = (BSJ *)malloc(sizeof(BSJ));
		...
		sscanf(message->status + strlen(bs_resp_found) + 1,
		    "%" PRId64 " %lu", &job->id, &job->size);
		job->data = message->data;
		message->data = 0;
		bs_free_message(message);
...
int
bs_peek(int fd, int64_t id, BSJ **job)
{
	char command[512] = { 0 };
...
int
bs_peek_ready(int fd, BSJ **job)
{
	return bs_peek_job(fd, "peek-ready\r\n", job);
}

int
bs_peek_delayed(int fd, BSJ **job)
{
	return bs_peek_job(fd, "peek-delayed\r\n", job);
}

int
bs_peek_buried(int fd, BSJ **job)
{
	return bs_peek_job(fd, "peek-buried\r\n", job);
}

int
bs_kick(int fd, int bound)
{
	BSM *message;
	char command[512] = { 0 };
...
int
bs_list_tube_used(int fd, char **tube)
{
	BSM *message;
	char command[64] = { 0 };
...
int
bs_get_info(int fd, char *command, char **yaml)
{
	BSM *message;
	size_t size;
...
int
bs_list_tubes(int fd, char **yaml)
{
	char command[64] = { 0 };
...
int
bs_list_tubes_watched(int fd, char **yaml)
{
	char command[64] = { 0 };
...
int
bs_stats_job(int fd, int64_t id, char **yaml)
{
	char command[128] = { 0 };
...
int
bs_stats_tube(int fd, const char *tube, char **yaml)
{
	char command[512] = { 0 };
...
void
bs_version(int *major, int *minor, int *patch)
{
