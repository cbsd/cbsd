...
updateRedis()
{
	...
	if (index > 1) {
		int rc = redis_do((index == 5 ? "HMSET" : "HSET"),
		    (index == 5 ? CR_INLINE : CR_INT), 0, index, _params, 0,
		    NULL);
		if (rc != 0)
			printf("Redis returned %i\n", rc);
...
doWork(void *param)
{
	...
	for (;;) {
		if (sysctlbyname("kern.cp_time", &cur, &cur_sz, NULL, 0) < 0) {
			tolog(log_level,
			    "Error reading kern.cp_times sysctl\n");
			pthread_exit(NULL);
			exit(-1);
		}
		...
		util = 100.0L -
		    (100.0L * cur[CP_IDLE] / (sum ? (double)sum : 1.0L));
		pcpu = (int)util;
		pmem = get_pmem();

		size_t len = sizeof(temperature);
		sysctlbyname("dev.cpu.0.temperature", &temperature, &len, NULL,
...
main(int argc, char **argv)
{
	...
	if (path_my_pidfile == NULL) {
		asprintf(&path_my_pidfile, "%sracct-hoster-statsd.pid",
		    _PATH_VARRUN);
		if (path_my_pidfile == NULL) {
			printf("asprintf");
			exit(1);
	...
	if (pidfile == NULL) {
		if (errno == EEXIST) {
			printf("racct-hoster-statsd already running, pid: %d.",
			    otherpid);
			exit(1);
		}
		if (errno == EAGAIN) {
...
#endif
	{
		...
		sysctlbyname("vm.stats.vm.v_page_size", &page_size, &len, NULL,
		    0);
	}
...
get_pmem()
{
	...
	i = sysctlbyname("vm.stats.vm.v_page_size", &page_size, &page_size_len,
	    NULL, 0);
	...
	i = sysctlbyname("vm.stats.vm.v_active_count", &active_count,
	...
	i = sysctlbyname("vm.stats.vm.v_wire_count", &wire_count,
