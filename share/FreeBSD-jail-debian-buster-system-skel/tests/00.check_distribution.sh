...
if [ ! -f ${rootfs_dir}/bin/bash ]; then
	...
	if getyesno "Shall i download distribution via deboostrap from ${SRC_MIRROR}?"; then
		...
		for i in ${FASTEST_SRC_MIRROR}; do
			${ECHO} "${N1_COLOR}${BASH_CMD} ${DEBOOTSTRAP_CMD} ${H5_COLOR}--include=openssh-server,locales,rsync,sharutils,psmisc,patch,less,apt --components main,contrib ${H3_COLOR}buster ${N1_COLOR}${rootfs_dir} ${i}${N0_COLOR}"
			/bin/sh <<EOF
${BASH_CMD} ${DEBOOTSTRAP_CMD} --include=openssh-server,locales,rsync,sharutils,psmisc,patch,less,apt --components main,contrib --arch=amd64 --no-check-gpg buster ${rootfs_dir} ${i}
EOF
			ret=$?
			[ ${ret} -eq 0 ] && break
		done
