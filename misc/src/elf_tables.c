...
usage(void)
{
	...
	printf(
	    "  usage: elf_tables [--osname] [--ver] [--freebsdver] [--arch] [--wordzize] <path-to-file>\n");
	printf("     --osname - show OS\n");
	printf("     --ver - show version value\n");
	printf(
	    "     --freebsdver - show FreeBSD version value ( --ver / 1000000 )\n");
	printf("     --arch - show ARCH\n");
	printf("     --wordsize - show wordsize\n");
	printf("\n");
	printf(
	    "  example: elf_tables --osname --freebsdver --arch --wordsize /bin/sh\n");
	printf("\n");
	printf(
	    "hint: output order always is -> osname:ver:freebsdver:arch:wordsize\n");
	exit(0);
}
...
pkg_get_myabi(char *myfile, char *dest, size_t sz)
{
	...
	case EM_MIPS:
		...
		// not ready for mips
		// snprintf(dest + strlen(dest), sz - strlen(dest),
		// ":%s:%s:%s:%s", arch, wordsize_corres_str, endian_corres_str,
		// abi);
		break;
	default:
		if (show_arch) {
...
main(int argc, char *argv[])
{
	...
	if ((!show_arch) && (!show_ver) && (!show_freebsdver) &&
	    (!show_osname) && (!show_wordsize)) {
		show_arch = 1;
		show_ver = 1;
		show_freebsdver = 1;
		show_osname = 1;
