...
// Раз мы позволяем ввести пользователю нужный тип из командной строчки в виде
// строки, нужен механизм для поиска значения оригинального define по его
// написанию. Эта структура учавствует в поиске в функции magicIndex
const struct mimeType mime_info[] = {
	{ MAGIC_NONE, "MAGIC_NONE" }, { MAGIC_DEBUG, "MAGIC_DEBUG" },
	{ MAGIC_SYMLINK, "MAGIC_SYMLINK" },
	{ MAGIC_COMPRESS, "MAGIC_COMPRESS" },
	{ MAGIC_DEVICES, "MAGIC_DEVICES" },
	{ MAGIC_MIME_TYPE, "MAGIC_MIME_TYPE" },
	{ MAGIC_CONTINUE, "MAGIC_CONTINUE" }, { MAGIC_CHECK, "MAGIC_CHECK" },
	{ MAGIC_PRESERVE_ATIME, "MAGIC_PRESERVE_ATIME" },
	{ MAGIC_RAW, "MAGIC_RAW" }, { MAGIC_ERROR, "MAGIC_ERROR" },
	{ MAGIC_MIME_ENCODING, "MAGIC_MIME_ENCODING" },
	{ MAGIC_MIME, "MAGIC_MIME" }, { MAGIC_APPLE, "MAGIC_APPLE" },
...
main(int argc, char *argv[])
{
	...
	while (TRUE) {
		...
		case C_METHOD:
			...
			if (method == 0) {
				...
				warn(
				    "Magic index not found for %s, forcing to MAGIC_DEVICES\n",
				    tmp_method);
			}
			free(tmp_method);
			break;
	...
	// init cookie, can be logical operand, e.g: "
	// MAGIC_DEVICES|MAGIC_NO_CHECK_COMPRESS|MAGIC_NO_CHECK_ENCODING "
